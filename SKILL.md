...
Create one restrained bitmap artwork that feels physically assembled from hand-cut patterned paper on a journal page. Favor a structurally readable motif, generous negative space, simple muted paper colors or familiar patterns, and believable torn edges over dense scrapbook decoration. The collage is completely flat and front-facing—every paper piece is a flat solid-color cutout with zero perspective, zero 3D volume, and zero curvature. Never render a glass as a 3D cylinder or a mug as a 3D cup; use flat front silhouettes (a vertical rectangle for a tall glass, a rounded shape for a mug). No ellipse rims, no glass thickness, no inner depth. Each paper piece has one uniform solid color—no gradients, highlights, painted shadows, airbrush, watercolor wash, or tonal variation within a piece. Depth and form come ONLY from stacking multiple flat pieces (visible overlap edge seams) and from choosing different flat color shades. No sculptural depth, no cast shadows.
...
- For image-based work, default to NO text. Add a concise factual English title only when the user explicitly requests it. When requested, derive one to three words from the visible image content, use clear faded typewriter lettering at a 26–32 pt equivalent with 28 pt as the default, and never overlap the motif. Use moderately dark faded ink, adequate tracking, and immediate legibility while keeping the title harmonious with the composition. Exact user-supplied text overrides the summary.
- Generate the transparent motif without text. When a caption is requested, add it deterministically with `scripts/compose_direct_split.py --motif <MOTIF> --caption "<TITLE>"`. The compositor places the title close to, but outside, the motif by default; use explicit caption coordinates only when art direction requires them. If the generated motif has a solid white background instead of a pre-made alpha channel, pass `--motif-white-bg` and the compositor will remove the white background automatically—no separate cutout step needed.
...
## Image generation fallback chain (photo transformations)
...
- The collage reads as a flat journal clipping with shallow layers, not a volumetric paper sculpture. Match the preferred reference quality through approximately 12–18+ coherent broad or medium hand-cut paper pieces, adjusted to subject structure, with stacked planes, soft fibrous torn edges, and mild analog pigment variation. Preserve perceived volume when useful through adjacent light, middle, and dark paper faces, overlap order, and negative-space cuts without creating inflated or sculptural paper depth. Do not add gray graphite, pencil, pen, or ink outlines; communicate rims, handles, stems, grids, and structural edges through paper silhouettes, overlap seams, negative space, narrow paper strips, or simple paper patterns. Each paper piece must have one uniform solid color with no internal gradients, highlights, or painted shadows; depth comes only from stacking and color choice. Major paper pieces may carry a low-contrast embedded flat pattern (fine dots, narrow stripes, light grid, washi-paper grain) that is uniform across the piece with no tonal variation. Solid paper and familiar patterns such as stripes, checks, grids, florals, and dots still dominate. Natural paper fiber, slight mottling, and overlap darkening are desirable; digital gradients, glossy vector fills, photographic texture, and ornate multicolor prints are not. Keep the combined area of any necessary fragmented color patches below 70% of the collage-motif region. No cast shadows or drop shadows inside the motif. No coasters, bases, stands, pedestals, or added 3D props.
...
- For image-based work, no text is present by default. If the user requested text, it is exact and immediately legible. Description-only work remains text-free by default.
